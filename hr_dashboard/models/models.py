...
class HrDashboard(models.Model):
    ...
    def get_employee_info(self):
        ...
        timesheet_search_view_id = self.env.ref('hr_timesheet_sheet.view_hr_timesheet_sheet_filter')
        job_search_view_id = self.env.ref('hr_recruitment.view_crm_case_jobs_filter')
        attendance_search_view_id = self.env.ref('hr_attendance.view_hr_attendance_filter')
        expense_search_view_id = self.env.ref('hr_expense.view_hr_expense_filter')
        ...
        attendance_today = self.env['hr.attendance'].sudo().search_count([('name', '>=',
                            str(datetime.datetime.now().replace(hour=0, minute=0, second=0))),
                            ('name', '<=', str(datetime.datetime.now().replace(hour=23, minute=59, second=59)))])
        expenses_to_approve = self.env['hr.expense'].sudo().search_count([('state', 'in', ['submit'])])
        ...
        query = """
            select sum(a.worked_hours) as worked_hours, e.name_related as employee
            from hr_attendance a
            inner join hr_employee e on(a.employee_id = e.id)
            group by e.name_related
        """
        cr.execute(query)
        ...
        query = """
            select e.name_related as employee, j.name as job, d.name as department,
